...
function NewsFeedInner({ data, selectedEntity, regionDossier, regionDossierLoading }: { data: DashboardData, selectedEntity?: SelectedEntity | null, regionDossier?: RegionDossier | null, regionDossierLoading?: boolean }) {
    ...
    // Threat bucket selected from 3D globe (country aggregate)
    if (selectedEntity?.type === 'threat_bucket') {
        const country = selectedEntity.extra?.country || String(selectedEntity.id || '').toUpperCase();
        const count = selectedEntity.extra?.count ?? 0;
        return (
            <motion.div
                initial={{ y: 50, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.3 }}
                className="w-full bg-black/60 backdrop-blur-md border border-red-800 rounded-xl flex flex-col z-10 font-mono shadow-[0_4px_30px_rgba(255,80,80,0.25)] pointer-events-auto overflow-hidden flex-shrink-0"
            >
                <div className="p-3 border-b border-red-500/40 bg-red-950/40 flex justify-between items-center">
                ...
                <div className="p-4 text-[10px] text-[var(--text-secondary)] space-y-2">
                    <div>
                        <span className="text-[8px] tracking-[0.18em] text-[var(--text-muted)] block mb-1">
                            AGGREGATED INDICATORS
                        </span>
                        <span className="text-2xl font-bold text-red-400">{count.toLocaleString()}</span>
                        <span className="ml-2 text-[9px] text-[var(--text-muted)]">recent OSINT hits mapped to this country</span>
                    </div>
                    <p className="text-[9px] leading-relaxed text-[var(--text-muted)]">
                        This dot represents a bucket of IP / URL indicators from AbuseIPDB and OpenPhish with geolocation resolving to this country.
                        Use the THREAT FEED and FIND / LOCATE panel on the right to inspect individual indicators.
                    </p>
                </div>
            </motion.div>
        );
        ...
        >
            ...
            >
                <div className="flex justify-between items-center relative z-10">
                    <h2 className="text-xs tracking-widest font-bold text-cyan-400 flex items-center gap-2">
                        <AlertTriangle size={14} /> GLOBAL THREAT INTELLIGENCE
                    </h2>
