...
import { Search, Crosshair, Plane, Shield, Star, Ship, X, Database, AlertTriangle } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { trackedOperators } from '../lib/trackedData';

export interface ThreatIndicatorOption {
    value: string;
    type: string;
    source?: string;
    lat?: number;
...
export default function FindLocateBar({ data, indicators = [], onLocate, onFilter, onSelectThreatIndicator }: FindLocateBarProps) {
    ...
    const allEntities = useMemo(() => {
        ...
        // Threat indicators (OSINT)
        for (const ind of indicators) {
            const lat = ind.lat ?? 0;
            const lng = ind.lng ?? 0;
            results.push({
                id: `threat-${ind.type}-${ind.value}`,
                label: ind.value,
                sublabel: `${ind.type} · ${ind.source || "OSINT"}`,
                category: "THREAT",
    ...
    const handleSelect = (result: SearchResult) => {
        ...
        } else if (result.entityType === "threat_indicator") {
            onSelectThreatIndicator?.(result.label);
            if ((result.lat !== 0 || result.lng !== 0) && result.lat != null && result.lng != null) {
    ...
    const categoryIcons: Record<string, React.ReactNode> = {
        ...
        THREAT: <AlertTriangle size={10} className="text-red-400" />,
    };
        ...
        <div ref={containerRef} className="relative w-full pointer-events-auto">
            <div className="flex items-center gap-2 bg-[var(--bg-primary)]/40 backdrop-blur-md border border-[var(--border-primary)] rounded-lg px-3 py-2 focus-within:border-cyan-500/40 transition-colors">
                ...
                <input
                    ...
                    type="text"
                    value={query}
                    placeholder="Find threat indicator, IP, URL or location..."
            ...
            <AnimatePresence>
                ...
                {isOpen && query.trim() && filtered.length === 0 && (
                    ...
                    >
                        <div className="text-[9px] text-[var(--text-muted)] font-mono tracking-widest">NO MATCHES</div>
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
