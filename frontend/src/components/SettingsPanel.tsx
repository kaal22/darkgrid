...
const SettingsPanel = React.memo(function SettingsPanel({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    const [adminKey, setAdminKey] = useState(() => {
        if (typeof window !== 'undefined') return localStorage.getItem('DarkGrid_admin_key') || '';
        return '';
    });
    ...
    // OSINT backend: keys live in backend .env only; no /api/settings/api-keys
    const fetchKeys = useCallback(async () => {
        setApis([]);
    }, []);

    // OSINT build: no news-feeds API on backend
                        ...
                        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[var(--border-primary)]/40 bg-[var(--bg-primary)]/30">
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    localStorage.setItem('DarkGrid_admin_key', e.target.value);
                                }}
                            ...
                            <>
                                ...
                                <div className="mx-4 mt-4 p-3 rounded-lg border border-cyan-900/30 bg-cyan-950/10">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <p className="text-[10px] text-[var(--text-secondary)] font-mono leading-relaxed">
                                            API keys are set in the <span className="text-cyan-400">backend .env</span> file only. This UI does not store or read keys.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 overflow-y-auto styled-scrollbar p-4 space-y-3">
                                    <div className="rounded-lg border border-cyan-900/50 bg-cyan-950/10 p-4">
                                        <p className="text-[10px] font-mono text-cyan-400 font-bold mb-2">Backend .env (in the repo backend folder)</p>
                                        <ul className="text-[10px] font-mono text-[var(--text-secondary)] space-y-1">
                                            <li><span className="text-cyan-300">ABUSEIPDB_API_KEY</span> — optional; get at abuseipdb.com for IP abuse data</li>
                                            <li>OpenPhish feed runs with no key</li>
                                        </ul>
                                        <p className="text-[10px] font-mono text-[var(--text-muted)] mt-3">Frontend: set <span className="text-cyan-300">NEXT_PUBLIC_THREAT_API_URL=http://localhost:8000</span> in <span className="text-cyan-300">.env.local</span> for local dev.</p>
                                    </div>
                                    {Object.keys(grouped).length > 0 && Object.entries(grouped).map(([category, categoryApis]) => {
                            ...
                            <>
                                ...
                                <div className="mx-4 mt-4 p-3 rounded-lg border border-orange-900/30 bg-orange-950/10">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <p className="text-[10px] text-[var(--text-secondary)] font-mono leading-relaxed">
                                            This build uses the threat feed from the OSINT backend only. News feeds API is not available.
                                        </p>
                                    </div>
                                </div>
