...
import { useThreatDataPolling } from "@/hooks/useThreatDataPolling";
import { useReverseGeocode } from "@/hooks/useReverseGeocode";
import { useRegionDossier } from "@/hooks/useRegionDossier";
import ThreatFeed from "@/components/ThreatFeed";

// 3D threat globe (DarkGrid OSINT) — replace MaplibreViewer for threat-intel dashboard
const ThreatGlobeViewer = dynamic(() => import('@/components/ThreatGlobeViewer'), { ssr: false });
...
export default function Dashboard() {
  ...
  const { indicators: threatIndicators, status: threatStatus } = useThreatDataPolling();
  ...
  // Apply threat filters to indicators (type, source, severity)
  const filteredThreatIndicators = useMemo(() => {
    const list = threatIndicators ?? [];
    const typeFilter = activeFilters.indicator_type;
    const sourceFilter = activeFilters.indicator_source;
    const severityFilter = activeFilters.indicator_severity;
    if (!typeFilter?.length && !sourceFilter?.length && !severityFilter?.length) return list;
    const severityToLabel = (s: number | null) => (s == null ? "low" : s >= 3 ? "high" : s >= 2 ? "medium" : "low");
    return list.filter((ind) => {
      if (typeFilter?.length && !typeFilter.includes(ind.type)) return false;
      if (sourceFilter?.length && !sourceFilter.includes(ind.source?.toLowerCase())) return false;
      if (severityFilter?.length) {
        const label = severityToLabel(ind.severity ?? null);
        if (!severityFilter.includes(label)) return false;
      }
      return true;
    });
  }, [threatIndicators, activeFilters]);
  ...
  return (
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-primary)] overflow-hidden font-sans">
      ...
      {/* 3D THREAT GLOBE (DarkGrid OSINT) */}
        ...
        <>
          ...
          >
            ...
            <div className="flex flex-col">
              <h1 className="text-2xl font-bold tracking-[0.4em] text-[var(--text-primary)] flex items-center gap-3" style={{ fontFamily: 'monospace' }}>
                D A R K <span className="text-cyan-400">G R I D</span>
              </h1>
              <span className="text-[9px] text-[var(--text-muted)] font-mono tracking-[0.3em] mt-1 ml-1">GLOBAL THREAT INTELLIGENCE</span>
            </div>
          </motion.div>
          ...
          >
            ...
            <ErrorBoundary name="WorldviewLeftPanel">
              <WorldviewLeftPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} threatCount={filteredThreatIndicators?.length ?? 0} onSettingsClick={() => setSettingsOpen(true)} onLegendClick={() => setLegendOpen(true)} gibsDate={gibsDate} setGibsDate={setGibsDate} gibsOpacity={gibsOpacity} setGibsOpacity={setGibsOpacity} onEntityClick={setSelectedEntity} onFlyTo={(lat, lng) => setFlyToLocation({ lat, lng, ts: Date.now() })} trackedSdr={trackedSdr} setTrackedSdr={setTrackedSdr} />
            </ErrorBoundary>
          </motion.div>
          ...
          >
            ...
            {/* THREAT INTEL FEED (DarkGrid OSINT) */}
            <div className="flex-shrink-0">
              <ThreatFeed
                indicators={filteredThreatIndicators}
                status={threatStatus}
                onSelectIndicator={(value) => setSelectedEntity({ type: "threat_indicator", id: value, extra: { value } })}
              />
            </div>
            ...
            <div className="flex-shrink-0">
              <FindLocateBar
                data={data}
                indicators={filteredThreatIndicators?.map((ind) => ({ value: ind.value, type: ind.type, source: ind.source })) ?? []}
                onLocate={(lat, lng) => setFlyToLocation({ lat, lng, ts: Date.now() })}
                onSelectThreatIndicator={(value) => setSelectedEntity({ type: "threat_indicator", id: value, extra: { value } })}
      ...
      {threatStatus === 'disconnected' && (
        <div className="absolute top-0 left-0 right-0 z-[9000] flex items-center justify-center py-2 bg-red-950/90 border-b border-red-500/40 backdrop-blur-sm">
          <span className="text-[10px] font-mono tracking-widest text-red-400">
            THREAT API OFFLINE — Cannot reach the OSINT backend. Check that the backend is running and NEXT_PUBLIC_THREAT_API_URL is set.
          </span>
        </div>
      )}
